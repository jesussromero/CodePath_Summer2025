...
def total_treasures(treasure_map):
    return sum(treasure_map.values()) # Works
...
write a function can_trust_message() that returns True if the message contains every letter of the English alphabet at least once, and False otherwise.
...
def find_duplicate_chests(chests):
    ...
    return ans # Works
'''
My solution using a set that also works:
def find_duplicate_chests(chests):
    tracking_set = set()
    answer = []
    for num in chests:
...
def is_balanced(code):
    # we basically need all frequencies except for one which at most has to be greater by 1
    # use dict 
    my_dict = {}
    # Frequency map 
    for letter in code:
    ...
    x = set()
    # Adds all values to a set allowing only two different values to be in it: One which
    # represents most frequencies, and the second which represents only the last frequency
    for key, value in my_dict.items():
        x.add(value)

    result = list(x)

    # The length of the list can't be greater than two, we calculate the absolute value of the difference
    # which at most can be 1
    if len(result) == 2:
...
Assume that each input has exactly one solution, and you may not use the same location twice.
...
def find_treasure_indices(gold_amounts, target):
    # We declare a dictionary that will be used to store the key (number), value (index) combination
    dict = {}
    # We use ennumerate to keep track of the index and value at each iteration
...
def organize_pirate_crew(group_sizes):
    result = []
    helper_dict = {}
    for i, value in enumerate(group_sizes):
        if value not in helper_dict:
            helper_dict[value] = [i]
        else:
            helper_dict[value].append(i)
    
    for size, pirates in helper_dict.items():
...
The logs are represented by a 2D integer array logs where each logs[i] = [pirateID, time] indicates that the pirate with pirateID performed an action at the minute time.

Multiple pirates can perform actions simultaneously, and a single pirate can perform multiple actions in the same minute.

The pirate action minutes (PAM) for a given pirate is defined as the number of unique minutes in which the pirate performed an action.
